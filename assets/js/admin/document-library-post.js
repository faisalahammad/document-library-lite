...
jQuery(function ($) {
  ...
  dlwDocumentLink.prototype.initMetaboxLocking = function () {
    ...
    if ($metabox.length) {
      ...
      $metabox.removeAttr('draggable').removeClass('ui-sortable-handle');
      $metabox.find('*').removeAttr('draggable').removeClass('ui-sortable-handle');
      $metabox.find('.postbox-header h2').css('cursor', 'default');
      $metabox.find('.handle-order-higher, .handle-order-lower').remove();
      $metabox.off('.dlp-lock').on('mousedown.dlp-lock dragstart.dlp-lock drag.dlp-lock', function (e) {
        // Allow specific interactive elements
        if ($(e.target).is('input, select, button, textarea, option, .handlediv') || $(e.target).closest('input, select, button, textarea, .handlediv').length) {
          return true;
        }
